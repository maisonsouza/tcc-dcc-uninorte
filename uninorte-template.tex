%=============================================================%
% Modelo de TCC do Curso de Ciência da Computação do UniNorte
% Desenvolvido por:
%   Prof. Afonso Costa (prof.afcosta@gmail.com)
%   Profa. Tereza Ramos
% Versão: 0.1
%=============================================================%

\documentclass[12pt,oneside,openright,final,brazil,a4paper,top=3.5cm,left=3cm,
right=3cm,bottom=2.5cm]{memoir}
\usepackage[brazilian]{babel}
\usepackage[latin1]{inputenc}
\usepackage{setspace}
\usepackage{graphicx}
\usepackage[lined, boxed, portugues]{algorithm2e}

\input{confpage.tex}

\sloppy

\newcommand{\tableformat}{\small\centering}
\newcommand{\figureformat}{\centering}

\newcommand{\tittcc}{
    \textsf{\bfseries\LARGE Modelo de TCC do Curso de Ciência da\\
    \vspace{0.8ex}
    Computação do UniNorte}
}

\newcommand{\descrtcc}{
\hspace{\stretch{1}}\parbox{0.51\textwidth}{
Trabalho de Conclusão de Curso apresentado à Coordenação de Ciência da
Computação do Centro Universitário do Norte, como parte dos requisitos
necessários para a obtenção do título de Bacharel em Ciência da Computação.

Orientador: Nome do Orientador.
}}

\begin{document}

\frontmatter
\pagestyle{empty}

% -- Capa ---------------------------------------------------------------------
{\centering
\textsf{\large
CENTRO UNIVERSITÁRIO DO NORTE - UNINORTE\\
Laureate International Universities\\
\vspace*{\stretch{0.01}}
CURSO DE CIÊNCIA DA COMPUTAÇÃO
}

\vspace*{\stretch{0.2}}

{\large NOME DO AUTOR 1\\}
{\large NOME DO AUTOR 2\\}
{\large NOME DO AUTOR 3\\}

\vspace*{\stretch{0.3}}

\tittcc

\vspace*{\stretch{1}}

MANAUS \\
2010

}
\cleardoublepage

% -- Folha de rosto -----------------------------------------------------------
{\centering
{\large NOME DO AUTOR 1\\
        NOME DO AUTOR 2\\
        NOME DO AUTOR 3\\
}

\vspace*{\stretch{1}}

\tittcc

\vspace*{\stretch{0.5}}

\descrtcc

\vspace*{\stretch{1.5}}

MANAUS\\
2010

}
\cleardoublepage

% -- Ficha Catalográfica ---------------------------------------------------
\begin{center}
{\Large Ficha Catalográfica}
\cleardoublepage
\end{center}

% -- Banca Examinadora --------------------------------------------------------
{\centering

{\large NOME DO AUTOR 1\\}
{\large NOME DO AUTOR 2\\}
{\large NOME DO AUTOR 3\\}

\vspace*{\stretch{2}}

\tittcc

\vspace*{\stretch{1}}

\descrtcc

\vspace*{\stretch{1}}

\begin{center}
\large Banca Examinadora
\end{center}

\vspace{0,5cm}
........................................................................\\
Prof. Nome do Professor(a) -- Orientador\\
Instituição do professor \\
\vspace{0,5cm}
........................................................................\\
Prof. Nome do Professor(a)\\
Instituição do professor \\
\vspace{0,5cm}
........................................................................\\
Prof. Nome do Professor(a)\\
Instituição do professor \\

\vspace*{\stretch{0.5}}

\begin{center}
Manaus\\
2010
\end{center}
}
\cleardoublepage

% -- Dedicatória --------------------------------------------------------------
\vspace*{\stretch{2}}

\hspace{\stretch{1}}\textit{Texto da dedicatória}\hspace{1cm}

\vspace*{\stretch{1}}

\cleardoublepage

% -- Agradecimentos -----------------------------------------------------------

\chapter*{Agradecimentos}
\thispagestyle{empty}

{\setlength{\parindent}{0cm}

Texto dos agradecimentos.
}
\cleardoublepage

% -- Epígrafe -----------------------------------------------------------------

{\vspace*{\stretch{1}}

\hspace{\stretch{1}}\parbox{0.5\textwidth}{%
Texto da epígrafe.
}
\vspace{1em}

\hspace{\stretch{1}}{\emph{Autor}}
}
\cleardoublepage

% -- Resumo -------------------------------------------------------------------

\chapter*{Resumo}
\thispagestyle{empty}

{Resumo do trabalho deve ser colocado aqui.

\vspace*{\stretch{1}}

\noindent \textsf{Palavras-chave:} Lista de palavras separadas por vírgula.
}
\cleardoublepage

% -- Abstract -----------------------------------------------------------------

\chapter*{Abstract}
\thispagestyle{empty}

{Abstract's text.

\vspace*{\stretch{1}}

\noindent \textsf{Keywords:} Words
}
\cleardoublepage

% -- Índices ------------------------------------------------------------------
\pagenumbering{roman}
\pagestyle{plain}

\listoffigures*    % gera a lista de figuras
\cleardoublepage

\listoftables*     % gera a lista de tabelas
\cleardoublepage

\tableofcontents*  % gera o índice (sumário)
\cleardoublepage

\mainmatter
\pagestyle{myheadings}
\let\chaptermark=\mychaptermark
\let\sectionmark=\mysectionmark

\chapter*{Introdução}
\label{cap:cap_1}
\addcontentsline{toc}{chapter}{Introdução}
\input{cap1}

\chapter{Fundamentação Teórica}
\label{cap:cap_2}
\input{cap2}

\chapter{Desenvolvimento}
\label{cap:cap_3}
\input{cap3}

\chapter*{Considerações Finais}
\label{cap:cap_4}
\addcontentsline{toc}{chapter}{Considerações Finais}
\input{cap4}

\cleardoublepage
\renewcommand{\bibname}{Referências}
\pagestyle{bibliography}
\bibliographystyle{apalike}
\bibliography{uninorte-template}
\end{document}
